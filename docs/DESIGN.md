...
按用户澄清，右侧终端标签保留原有三行圆角描边、无填充样式，只压缩横向留白：新增按钮仅 `+`，agent 名称贴合内容，过长按终端列宽截断并加省略号。名称取各 tab 当前焦点窗格的请求目标/已接入 agent；无 agent 时显示 Empty，不显示 Tab N。当前 tab 使用亮灰白边框、加粗名称，并在顶边居中显示一个小 `●` 标记；非当前边框保持暗灰，悬停不添加选中标记。× 独立命中，当前 tab 在可容纳时始终可见，翻页箭头保留；内容/PTY/鼠标区域共同按三行标签条计算。此前单行方括号是主控误解，已撤回；弹框按钮不变。
