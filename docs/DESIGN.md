...
- Agents 的 `n`／New 打开启动表单，`o`／Open 打开接入位置菜单（1–6 对应当前、tab、左、右、上、下）。New 表单默认新 tab，Tab／Shift-Tab 切字段，Ctrl-U 清空，Ctrl-P／Next project 在已登记目录间轮换，也可手填；首条消息支持 Enter 和多行粘贴。打开位置用左右键或点击轮换。Ctrl-S／Start 提交，Esc／Ctrl-] 返回 Agents 并保留草稿；`n` 重新打开。执行中禁用重复提交和编辑，仍可返回、切换或关闭显示目标。
- 启动命令用 `shell-words` 做引号分组，直接传 argv，不经 shell，不执行变量展开／管道／重定向；预览使用相同 argv 的 shell 引号表示，支持 PgUp/PgDn／滚轮查看全文。不加 `--unique` 或模型／权限默认值；实际名称只取公开 start 返回的 `name`。成功后仍走公开 status 检查再 attach。
- `terminals` 保存 tab、递归等分布局、稳定窗格 ID 和请求修订号；`viewer` 保留单个终端生命周期。请求提交时预留目标（因此失败可以留下空 tab／窗格），替换同一目标会让旧结果失效；切换布局不改变请求归属。已关闭／过期目标的新 agent 继续托管并可从 Agents 接入。公开 status、start 和 PTY 创建均异步；关闭显示只把自有 attach 放入待回收集合，正常循环继续轮询退出，退出程序时收齐。
- 顶部 `+ Tab` 新增空 tab，箭头浏览超出条带的 tab，`×` 关闭对应 tab；窗格标题可聚焦，活动窗格底边提供 Close pane／Close tab（极窄时仅保留 × 关闭窗格）。最后一个 tab 关闭后留一个空 tab。内容区仍透传，原生控件与边框的点击不透传；停止保持独立确认入口。
- 分屏等分，横向不足 12 列或纵向不足 6 行时只绘制一支，活动窗格所在支优先；关系不丢失，放大后恢复，可从 Agents 再选已打开的 agent 跳入折叠窗格。不可见 PTY 保留最后尺寸（初次后台创建为 80×24），可见时用实际内容区调整，内容区为空时不接收终端输入。tab 条带占右侧一行。

### T2 用户体验修订：New 表单（2026-09-26）

用户体验后表示 New 弹框不知如何操作；主控提出「选项目 → 选 Codex / Claude → 点击创建，名称自动生成并可修改，完整命令放高级设置」，用户回复「继续吧」，并强调：「还有输入框之类的体验，现在的很不好，不注意看都不知道是个输入框。而且点击输入框连个光标都没有」。以下覆盖本节前述 New 表单的展示和编辑方式，tab、split 和异步接入生命周期不变。

```text
New agent
Project   [ saddle                 ▾ ]
          /Users/…/saddle     [Edit path]
Agent     [● Codex] [○ Claude]
          ┌ Name ────────────────────┐
          │ saddle/codex▏            │
          └─────────────────────────┘
...
- 默认主流程为选项目、选 Codex/Claude、创建；默认当前 Tasks 项目和 Codex，名称自动建议并可改，普通用户无需填写命令。项目显示当前目录和明确的选择入口，从已登记目录选择，也保留手工编辑路径。选择仅影响此次创建的 cwd，不切换 Tasks 项目。沿用英文产品文案，上图中文说明仅为设计注释。
- 名称按项目和 agent 类型生成有效建议；用户手工改过后不因切换其他选项静默覆盖。自动命名可用公开 `corral start --unique` 避免重名，实际名称取返回 name；手工命名沿用精确名称及公开 CLI 的重名错误。此处覆盖上一版「不加 --unique」的实现取舍，不增加用户要理解的新开关。
- Advanced 默认折叠，容纳完整自定义命令、可选首条消息、打开位置（仍默认 New tab，保留现有六项）及完整命令预览。默认 Codex/Claude 分别运行 codex/claude；不自动加模型、effort 或权限参数。命令仍直接拆 argv，不经 shell。高级自定义值不得被无关焦点切换或收起/展开丢弃，参数最终来自用户看到的选择和输入。
...
- Project／Ctrl-P 展开仅供 New 使用的已登记目录列表（上下键、Enter 或鼠标选择）；Edit path／Ctrl-E 显示有框线的目录编辑框，不改变 Tasks 项目。空列表仍可手填路径。
- 默认 Codex，建议名为 `<目录末段>/<codex|claude>`；目录末段保留 ASCII 字母、数字、下划线、连字符，其余转连字符，空结果回退 `project`。目录或 agent 变化只更新未手改的建议名；建议名提交带 `--unique`，手改名不带。实际名称继续取 start 返回 name。
- 明确选择 Codex／Claude 会把命令替换为对应裸命令；自定义命令显示 Custom command。Tab 聚焦选项不等于选择，只有点击／Enter／空格／左右键明确选择才改变值；收起 Advanced 或切焦点不丢弃高级值。F4 展开／收起，打开位置仍默认 New tab，保留六项。
- 编辑器仅供 New 的四个文本框使用，以 UTF-8 字符边界保存插入位置，用终端显示列宽共同计算光标、点击和滚动。长行采用横向滚动、不软换行；消息按实际换行纵向滚动，Tab 字符显示为四格。Home／End 到当前行首尾，上下键按显示列移动；点击宽字符第二格也定位到该字符前。
- 小窗口随 Tab／Shift-Tab 或滚轮移动焦点而显示对应区域，底部创建／取消入口固定；Preview 聚焦时滚轮及 PgUp／PgDn 滚动全文。仅 Create agent／Ctrl-S 提交。字段无效时就地标示，提交还会展开并定位到无效字段；启动失败展示错误末尾最多三行并保留草稿。焦点位于 New 时禁止底层终端设置光标。
- 复用现有按钮、主题及异步 start 接线；未新增依赖、通用表单框架，也未修改 tab／split／PTY 生命周期。
...
## 28. Agents 选中条目样式（2026-09-26）

用户截图反馈选中项首行的白色竖线不好看，要求去掉并评估选中块左边加主题色长条是否更好。独立样式建议见 `docs/任务/Agents选中样式.md` 的方案 A/B；主控采纳 A：移除首行竖线，保留整块低对比背景，并加粗选中名称。

- 原因：当前每个条目已经有连续多行的选中背景；再加贯穿的侧条会紧挨树形连线，非分组末项时形成并排竖线，使层级显乱。背景和名称加粗足以表达选中，无须再借用状态/已接入/焦点的强调色。
- Agents 主行第 0 列保留空格，去掉 `▎`；树线和内容对齐不变。选中项名称保留亮色并加粗，其他文字、状态、effort、Git、已接入/未读标记沿用原色。
- 背景继续使用既有 `agent_selected`，覆盖选中项所有内容行（含折行、滚动后可见部分），条目间空行不铺。Agents 有无焦点不改变选中样式，焦点仍由面板边框表达。
- 本节覆盖第 13、14、18 节中 Agents 选中用左侧标记的旧描述。只改 Agents；Queue 的选中标记不在用户本轮指定范围，不扩展处理。不新增主题字段或更改默认颜色，不改交互。
- 实施顺序：待 New/Open 合并后，在更新到该 main 的独立分支实施，避免同时修改 src/ui.rs。

## 29. 位置优先的终端布局操作（2026-09-26）

用户指出 Show 前必然已选定 agent，质疑弹框意义。主控核对：鼠标点击列表已 attach，Show 的位置动作又因 activate_existing 提前返回，正常鼠标流程无法按所选位置分屏。用户已看过下列方案和文字示意，并明确回复「我觉得这个设计是可以的」。本节覆盖第 27 节 Open/Show 和直接创建空 tab 的旧交互；New 表单及第 28 节选中样式保持。

### 已批准的交互

- 左侧负责查看 agent：点击沿用当前打开/跳转行为，移除 Show in… 入口及旧 Show 弹框。New、Sort、Stop 等其他入口保持。
- 布局入口放到右侧：顶部 `＋` 添加标签页；当前窗格底部 `Split ▾` 打开紧凑的四向菜单（Left/Right/Above/Below），沿用终端主题、按钮、底部 Cancel Esc 风格，不再使用占满区域的大弹框。
- 先确定位置，再选择 agent。例如正在看 main，要把 review 放到右边：当前窗格 Split → Right → review。方向相对于发起操作的窗格。选择窗口只呈现该位置与候选 agent，选择完成才创建布局；取消恢复原样，不留下空窗格或空 tab。顶部 `＋` 同样先选 agent，选完再建 tab。
- 普通点击左侧已打开的 agent 仍跳转到现有位置；从新 tab/分屏选择窗口选择已打开的 agent，则移动现有会话到指定位置，并在候选行明确显示 `Move here`，不能再默默跳回旧位置。当前窗格自己的 agent 不列入本次分屏候选。
- New 保留现有体验和高级打开位置，不借本轮重做创建流程。无候选时清楚显示空态，保留取消入口，不借机创建真实 agent。
...
┌ Open agent on the right ──────────┐
│ dev-review                       │
│ dev-backend            Move here │
│                       ‹Cancel Esc›│
└──────────────────────────────────┘
```
...
移动的是现有显示会话，不复制接入或停止 agent；不得通过关闭再重连冒充移动。布局变化需要保持既有异步请求目标、输入隔离和关闭回收规则。来源空位、候选变化、发起窗格消失等局部实现取舍由开发者遵循现有布局规则最小处理并记录；若需改变已批准的用户操作流程，先报告。仅修改布局所需局部代码，不新增拖拽、持久化、主题配置或通用窗口框架。
...
- 发起窗格消失：弹框期间窗格只能由用户操作关闭，而弹框拦截所有输入，异步结果也不删除窗格，所以选定时发起窗格必然存在，不另加处理。New 表单的 Open in 六项与预留行为不变。

## 30. T14：弹框描边按钮与素雅 Tab（2026-09-26）

盘点和文字线框见 docs/任务/T14-按钮样式盘点.md。用户在明确询问是否实施后回复「不要待办，开干吧」，现按该方案实施；覆盖任务旧正文的“先记录/只探索”，不改变已有按钮功能。

- New 主表单的 Project、Edit path、Codex/Claude、Advanced、Open in，以及 Choose project 子页内部 Edit path：采用贴合文字的灰白细圆角框，无独立填充色，少量左右留白。保留选择标识、字段标签及原编辑光标体验。
- Split 四方向按钮同样描边，保留 2×2 排列；面板只增加按钮所需高度，底部 Cancel 仍单行紧凑。
- 所有弹框底部工具栏继续紧凑 ‹…›；Stop 确认底部 Cancel/Stop 统一到该样式，Stop 保留危险色。Queue 既有底栏、候选列表及输入框不改造，列表不加逐行按钮框。
- 右侧 Tab 与新增 tab 的加号采用三行细圆角描边、无额外背景；非当前 tab 使用低对比灰色，当前 tab 只将边框提亮至灰白、名称适度加粗，不常驻亮黄、粗框或大块底色。× 在各自框内独立命中；翻页箭头保持低调，Tab N 命名不变。
- 完整 tab 描边占三行，较原单行多两行；不额外增加留白或外框，终端内容/PTY 尺寸和命中几何共同按实际布局计算。小空间沿用现有裁剪/收缩原则，保证显示与命中一致，局部取舍由开发者记录。
- 复用现有按钮绘制与主题语义色，不全局替换所有 compact 按钮，不搭新主题或控件框架。悬停/焦点适度提亮、禁用弱化、危险语义保留；原有取消、创建、切 tab、关闭、移动和点击目标身份规则不变。只改显示及对应布局，不改 PTY 生命周期。
- 实现取舍（dev-t14-buttons）：弹框内部描边用 `buttons::draw_outlined_top`（沿用原三行圆角绘制，普通项边框为 muted 灰白，主要/危险/禁用色不变），底栏仍用 `draw_compact`；原三行按钮已无别处使用，改色不影响 Agents/Queue 主面板。New 键盘焦点原为焦点色加 selected 底色，三行框若沿用底色会成大色块，改为只把框和文字染成焦点色、不填底。New 各段高度随之调整（Project 5、Agent 4、Advanced 3、Open in 4），弹框高度 22→24、Advanced 38→40；Split 面板 6→9 行。Tab 条 3 行（`terminals::STRIP`），标签保持 `+ Tab`、`Tab N`；× 连同右侧留白和边框三列为关闭命中区，其余为切换区，悬停提亮只作用于所在命中区。翻页箭头仍为无框文字，放在中间行。视口不足 3 行或宽度放不下整个框时不画该控件也不留命中；新 tab 候选框从 tab 条下方开始。

### T14 放行后的小调整（2026-09-26，用户要求主控直接修改）

按用户澄清，右侧终端标签保留原有三行圆角描边、无填充样式，只压缩横向留白：新增按钮为 `+` 加左右各一格留白，避免过瘦；agent 名称贴合内容，过长按终端列宽截断并加省略号。名称取各 tab 当前焦点窗格的请求目标/已接入 agent；无 agent 时显示 Empty，不显示 Tab N。用户截图反馈后，移除顶边圆点，当前 tab 使用主题 focus 强调色细边框和加粗名称，非当前边框保持暗灰，均不填底。× 独立命中，当前 tab 在可容纳时始终可见，翻页箭头保留；内容/PTY/鼠标区域共同按三行标签条计算。此前单行方括号是主控误解，已撤回；弹框按钮不变。

- Tab 悬停/按下反馈覆盖完整圆角按钮：名称与 × 仍为独立点击目标，但二者任一被指向时使用同一整框高亮，避免左右半框异色；只调整绘制，不合并命中区域。
