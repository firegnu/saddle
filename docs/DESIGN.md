...
- 显示：主行在类型列（窄窗无类型列时在名称列）之后、状态文字之前放一个 3 列信号格。未知不显示图标，只留空白保持同列对齐；整个列表都没有已知 effort 时不占这一列，原有布局不变。
- 字形（T9 修订）：原先的整格方块 `▂▄▆` 偏厚重，改为盲文点阵画的细阶梯，每格只用左列点、右列天然留缝：亮的柱子由低到高 `⡄⡆⡇`（2、3、4 点高），没亮的位置只剩底部一点 `⡀`。三档形状各不相同：medium `⡄⡀⡀`、high `⡄⡆⡀`、xhigh `⡄⡆⡇`，不靠颜色也能数出柱数。盲文是终端常用字符（Unicode 宽度 1、非歧义宽度），不需要装字体或图标库。
- 颜色：亮柱用正文色，底点固定用弱化色（dim），不随选中或焦点变化。T7 曾让暗格在选中时随树线提亮为 muted（`Gray`），而多数终端配色里 `Gray` 与默认正文色几乎一样，选中后 high 看起来像满格；数据档位并没有变。现在选中、未选中、焦点在别处时图标完全一致。
