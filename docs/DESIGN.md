...
右侧终端标签条缩为单行无填充的灰色方括号边界按钮：`[+]`、`[agent名称 ×]`，不用三行框或 Tab N。名称取各 tab 当前焦点窗格的请求目标/已接入 agent；无 agent 时显示 Empty。长名称按终端列宽截断并加省略号，× 保持独立命中；变长后仍保证当前 tab 可见，翻页箭头保留。终端内容与鼠标/PTY 尺寸共同省回两行；弹框按钮不变。本条覆盖上面的三行 Tab 和 Tab N 历史方案。
