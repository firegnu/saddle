...
- 保留的行为：按项目前缀分组；working 有转圈；需要人处理的（blocked 等）醒目；窄屏时列表收为单行，身份信息集中在选中项详情中折行；列表放不下时有滚动条和「上下还藏着几个」的提示；回复区默认隐藏，按 `r` 显示选中 agent 的上一轮回复（`corral reply`）。
...
- 输入规则沿用第 5 节：Agents Tab→Queue，Shift-Tab→Viewer；Queue/Viewer Ctrl-]→Agents；Viewer 的 Tab/Shift-Tab 等均透传，不采用原型中互相矛盾的“三窗格 Tab 循环”。窄窗标签可点击，保留各面板选择和滚动状态。
