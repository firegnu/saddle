...
| 在格子之间切焦点 | 鼠标点格子；另给一组不会和里面程序冲突的键，实现时定（候选：`Ctrl-]` 后再按方向键或 Tab） |
| 退出 | 焦点在 Agents 时按 `q` |
...
- 终端解析：`alacritty_terminal` 或 `vt100`，第 1 步开工时先各写一个最小原型接 `corral attach` 跑一个 Claude Code 会话，比较显示是否正确（颜色、宽字符、光标、滚动），选一个并在设计里记下理由。
