...
## 2026-09-25：Claude Design 界面重设计（进行中）

- 已接受设计稿作为实现基准，技术栈不变；设计差异和真实数据边界见 DESIGN 第 13 节。
- 已完成基础：语义配色、按钮悬停/按下/释放命中、响应布局与输入底栏；布局与按钮行为 RED→GREEN。
- 已完成 Agents 扫描行/选中详情/回复，Queue 项目与任务分层、右侧状态列，原生项目/表单/详情/停止弹层。窄窗标签保持原有选中项与 Viewer 连接。
- 9 项假 CLI 整程序回归及 CJK/极小窗口渲染检查通过，clippy 通过。剩余收尾：异步焦点边界、帮助与 release 构建。
