...
## 2026-09-26：用户放行 T14 后直接调整终端标签（以本节为准）

- 用户表示 T14 已放行，并要求主控直接修改、不委派：右侧按钮过大，名称应与 agent 一致。已改为单行 `[+]`、`[agent名称 ×]`，长名按列宽截断；分屏时用当前焦点窗格名称。三行框与 Tab N 的旧方案被本次用户要求覆盖，说明见 DESIGN 第 30 节末尾。
- 检查：terminals 7 条（渲染、截断、焦点和命中），受标签变化影响 workflow 8 条均通过；Clippy、diff 检查通过。现有切换、移动、取消及会话归属逻辑未改；未扩旧 Queue/Stop 问题。
- 本轮直接在 main 小修并更新 release，无开发 worktree/agent，无任务队列操作，不执行 go/next。下一步等用户体验反馈；旧章节的 T14 待放行状态以用户已放行为准。
