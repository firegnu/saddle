...
**Show in… / o** chooses where to display the selected existing agent; **New** creates a new agent. The **Show agent** dialog names the agent and groups its actions into **Replace current pane**, **Open in new tab**, and **Split current pane** with Left/Right/Above/Below relative to the active terminal pane. Click an action or use its secondary shortcut (1–6 in that order). An agent already open anywhere is focused instead of attached twice. The bottom **Cancel Esc** button, Esc, or Ctrl-] closes the dialog without attaching. The **+ Tab** control creates a blank tab; the arrow controls reach tabs beyond the visible strip. Click a terminal's content or title to focus it. Closing a pane collapses its split; closing the final tab leaves an empty tab. Tab changes keep background attaches running. A start or attach belongs to the pane reserved at submission: switching tabs does not redirect it, and closing or replacing that target discards its attachment result without stopping a newly created agent. A failed start can leave its reserved pane empty. Tiny windows temporarily show only one branch where a split cannot fit, retaining the full layout for expansion; an empty terminal content area receives no input. Layouts last only for the current saddle run.
...
| Agents | o / Show in… | Choose where to display the selected existing agent |
