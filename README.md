...
Queue 的界面标签、帮助、表单和提示均为英文，任务标题/正文/原因及 CLI 输出保留原文。运行/执行中用蓝色，等待/暂停用琥珀色，空闲/就绪/完成用绿色，失败用红色；循环开启用绿色、关闭弱化。没有当前、待放行或待办时显示 `No active tasks`，仍保留历史记录；历史区底边固定在操作栏上方，显示可见范围、总数及末尾 `End` 标记。
